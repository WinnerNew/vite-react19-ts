...
export const authApi = {
  // 注册
  register: async (username: string, handle: string, password: string, avatar: string) => {
...
export const userApi = {
  ...
  // 更新用户资料
  updateProfile: async (bio: string, location: string, website: string, avatar: string) => {
...
export const postApi = {
  ...
  getPosts: async (limit: number = 10, offset: number = 0) => {
    const response = await request<{ posts: Post[]; total: number }>(`/post?limit=${limit}&offset=${offset}`);
    return response;
  },
...
export const messageApi = {
  ...
  getMessages: async (chatId: string) => {
    const response = await request<{ messages: Message[] }>(`/message/chats/${chatId}/messages`);
    return response.messages;
  },
  
  // 发送消息
  sendMessage: async (chatId: string, text: string) => {
    const response = await request<{ message: Message }>(`/message/chats/${chatId}/messages`, {
      method: 'POST',
