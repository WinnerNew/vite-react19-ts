...
import { Settings, Search, Loader2, UserPlus } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col bg-black min-h-full text-white">
      <header className="sticky top-0 bg-black/80 backdrop-blur-md z-40 border-b border-zinc-800">
        <div className="px-4 py-2 pt-[calc(env(safe-area-inset-top,0px)+1rem)]">
          <form onSubmit={handleSearch} className="flex items-center gap-4">
            ...
            <button type="button" className="p-2 hover:bg-zinc-800 rounded-full transition-colors text-zinc-100">
              <Settings size={20} />
            </button>
          </form>
        </div>
        ...
        <div className="flex flex-col">
          ...
          {searchResults.users.length > 0 && (
            <div className="p-4 border-b border-zinc-800">
              <h2 className="text-xl font-extrabold mb-4 tracking-tight">Users</h2>
              <div className="space-y-4">
                {searchResults.users.map(user => (
                  ...
                  >
                    <img src={user.avatar} className="w-10 h-10 rounded-full border border-zinc-800 object-cover" alt="avatar" />
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-[14px] text-zinc-100 truncate group-hover:underline">{user.username}</p>
                      <p className="text-zinc-500 text-[13px] truncate">{user.handle}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="flex flex-col">
            {searchResults.posts.length > 0 ? (
              <>
                <h2 className="text-xl font-extrabold p-4 tracking-tight">Posts</h2>
          ...
          <div className="border-t border-zinc-800 mt-4 p-4">
            ...
            <div className="space-y-4">
              {suggestions.length > 0 ? (
                suggestions.map(user => (
                  <div key={user.id} className="flex items-center gap-3 py-1">
                    ...
                    <div 
                      ...
                      onClick={() => navigate(`/profile/${user.id}`)}
                    >
                      <p className="font-bold text-[14px] text-zinc-100 truncate hover:underline">{user.username}</p>
                      <p className="text-zinc-500 text-[13px] truncate">{user.handle}</p>
                    </div>
                    <button 
                ...
                <p className="text-zinc-500 text-sm">No suggestions at the moment.</p>
              )}
            </div>
          </div>
        </>
      ...
      <ReplyModal 
        ...
        onReplySuccess={() => {
          if (isSearching) handleSearch({ preventDefault: () => {} } as any);
        }}
      />
    </div>
  );
