...
const ReplyModal: React.FC<ReplyModalProps> = ({
  ...
  const handleReply = async () => {
    ...
    try {
      const newReply = await postApi.createPost(text, undefined, post.id);
      setText("");
        ...
        >
          <header className="p-3 flex items-center justify-between border-b border-gray-800">
            ...
            <button
              ...
              onClick={handleReply}
              className={`bg-sky-500 text-white font-bold px-4 py-1.5 rounded-full transition-opacity ${!text.trim() || isSubmitting ? "opacity-50" : "opacity-100"}`}
            >
          ...
          <div className="flex-1 overflow-y-auto p-4">
            ...
            <div className="flex gap-3">
              ...
              <textarea
                ...
                className="flex-1 bg-transparent text-xl border-none outline-none resize-none pt-1 text-white"
                rows={6}
              />
            </div>
          </div>
